...
let shuffled2 = shuffle(jsquestions);
var answerShuffle;
start_btn.onclick =()=>{
...
total_q.innerText = shuffled2.length;
total_que_r.innerText = shuffled2.length;
...
function ShowQuestion(q_index){
    ...
    que_text.innerText = number+". "+ shuffled2[q_index].question;
    var option_statement = "";
    answerShuffle = shuffle(shuffled2[q_index].options)
    for(var i=0; i<answerShuffle.length; i++){
        option_statement += `<div class="option">${shuffled2[q_index].options[i]}</div>`;
    }
...
next_btn.onclick=()=>{
    ...
    if(shuffled2.length>que_index){
        ...
        percentage.innerText = ((right_answers*100)/shuffled2.length).toFixed(2)+"%";

        
    }

    if(shuffled2.length-1==que_index){
...
function UserAnswered(answer){
    ...
    let correctAns = shuffled2[que_index].answer;
